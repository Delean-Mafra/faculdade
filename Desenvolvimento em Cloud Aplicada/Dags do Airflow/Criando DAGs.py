# arquivo: atividade_pratica_dag.py
# DAG do Apache Airflow demonstrando PythonOperator, BashOperator, PostgresHook, XCom e BranchPythonOperator.
# Observação: configure a conexão no Airflow UI com:
# Conn Id: atividade_pratica_postgres
# Tipo: Postgres
# Host, Schema (database), Login, Password, Port conforme seu ambiente.
...
# -----------------------------
CONN_ID_POSTGRES = "atividade_pratica_postgres"  # certifique-se de criar esta conexão no Airflow
...
def imprimir_mensagem():
    """
    Task Python simples que imprime uma mensagem informativa.
    """
    print("Execução da task Python: operação simples concluída.")

def consultar_postgres_e_retornar_contagem() -> int:
    """
    Conecta ao PostgreSQL via PostgresHook, executa uma consulta de contagem
    e retorna o valor (o retorno será automaticamente enviado ao XCom).
    """
...
def decidir_ramificacao(**context):
    """
    Função usada pelo BranchPythonOperator.
    Lê o valor publicado no XCom pela task 'consultar_postgres' e decide
    qual caminho (task_id) deve ser executado.
    Retorna o task_id de destino (string).
    """
    ti = context["ti"]
