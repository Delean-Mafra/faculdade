...
# ==========================
# Diretório para artefatos intermediários
BASE_DIR = Path(os.getenv("ML_BASE_DIR", "data/ml_pipeline"))
BASE_DIR.mkdir(parents=True, exist_ok=True)

# Caminho do dataset de entrada (CSV) - por padrão usa data/train.csv
ARQUIVO_DADOS = Path(Variable.get("ML_INPUT_CSV_PATH", default_var=os.getenv("ML_INPUT_CSV_PATH", "data/train.csv")))
...
with DAG(
    ...
    def preprocessar_dados() -> Dict[str, str]:
        ...
        pd.DataFrame({TARGET: y_train}).to_csv(y_train_path, index=False, encoding="utf-8")
        pd.DataFrame({TARGET: y_test}).to_csv(y_test_path, index=False, encoding="utf-8")
    ...
    def treinar_modelo(paths: Dict[str, str]) -> str:
        X_train = pd.read_csv(paths["X_train"], encoding="utf-8")
        y_train = pd.read_csv(paths["y_train"], encoding="utf-8")[TARGET].squeeze()
    ...
    def validar_metricas(paths: Dict[str, str], modelo_path: str) -> Dict[str, Any]:
        X_test = pd.read_csv(paths["X_test"], encoding="utf-8")
        y_test = pd.read_csv(paths["y_test"], encoding="utf-8")[TARGET].squeeze()
    ...
    def selecionar_modelo(metricas: Dict[str, Any], modelo_path: str) -> Dict[str, str]:
        ...
        # Opcional: impor threshold (ex.: AUC >= 0.6) para aprovar o modelo.

        # Exemplo de threshold opcional:
    ...
    # Orquestração
    paths = preprocessar_dados()
    modelo_path = treinar_modelo(paths)
    metricas = validar_metricas(paths, modelo_path)
    selecionado = selecionar_modelo(metricas, modelo_path)
    publicar_modelo(selecionado)
